...
func (m *client) SendRequest(path string) ([]byte, error) {
	...
	if len(body) > maxResponseSize {
		return nil, fmt.Errorf("metadata response exceeds %d bytes", maxResponseSize)
	}
	return body, nil
}
...
func validateBaseURL(rawURL string) error {
	...
	if parsed.User != nil || parsed.RawQuery != "" || parsed.Fragment != "" {
		return fmt.Errorf("metadata URL must not contain user information, a query, or a fragment")
	}
	return nil
}
