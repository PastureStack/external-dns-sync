...
func TestSetFromEnvironmentBoundsTTLToSigned32BitRange(t *testing.T) {
	for _, test := range []struct {
		name string
		raw  string
		want int
	}{
		{name: "maximum", raw: "2147483647", want: 2147483647},
		{name: "overflow", raw: "2147483648", want: 300},
		{name: "large overflow", raw: "9223372036854775807", want: 300},
	} {
		t.Run(test.name, func(t *testing.T) {
			...
			t.Setenv("PLATFORM_SECRET_KEY", "test-secret-key")
			t.Setenv("ROOT_DOMAIN", "example.test")
			t.Setenv("TTL", test.raw)

			SetFromEnvironment()

			if TTL != test.want {
				t.Fatalf("TTL = %d, want %d", TTL, test.want)
			}
		})
	}
}
